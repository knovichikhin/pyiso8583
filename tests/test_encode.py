...
def test_input_type():
    ...
    with pytest.raises(
        TypeError,
        match="Decoded ISO8583 data must be dict, not bytes"):
...
def test_type_no_key():
    ...
    with pytest.raises(
        iso8583.EncodeError,
        match="Field data is required: field t"):
        iso8583.encode(doc_dec, spec=spec)
...
def test_bitmap_no_key():
    ...
    with pytest.raises(
        iso8583.EncodeError,
        match="Field data is required: field bm"):
        iso8583.encode(doc_dec, spec=spec)
...
def test_field_no_key():
    ...
    with pytest.raises(
        iso8583.EncodeError,
        match="Field data is required according to bitmap: field 2"):
        iso8583.encode(doc_dec, spec=spec)
...
def test_ebcidic_bitmaps():
    ...
    assert doc_enc['p']['len'] == b''
    assert doc_enc['p']['data'] == b'\xf8\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0'
    assert doc_dec['p'] == '8000000000000000'

    assert doc_enc['1']['len'] == b''
    assert doc_enc['1']['data'] == b'\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf0\xf8\xf0\xf0\xf0\xf0\xf0'
    assert doc_dec['1'] == '0000000000800000'
